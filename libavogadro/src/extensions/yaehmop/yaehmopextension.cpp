...
*/
  ...
  void YaehmopExtension::calculateBandStructure() const
  {
    ...
    for (size_t i = 0; i < numOrbitals; ++i) {
      ...
      for (size_t j = 0; j < numKPoints; ++j) {
        // x is k-point fraction. y is energy
        double x = static_cast<double>(j) /
                   static_cast<double>(numKPoints - 1.0) * x_multiplier;
        double y = bands[i][j];
    ...
    for (size_t i = 0; i < numSpecialKPoints; ++i) {
      // These should be evenly spaced out on the x axis since we
      // always connect them with t he same number of k points
      ...
      if (i == 0)
        ...
        kpointlabels_x.append(static_cast<double>(i) /
                              static_cast<double>(numSpecialKPoints - 1) *
                              x_multiplier);
      }
  ...
  QString YaehmopExtension::createYaehmopBandInput() const
  {
    if (!m_molecule) {
    ...
    YaehmopBandDialog d;
    if (!d.getKPointInfo(numKPoints, specialKPointString))
      return "";
